...
const TasksAndActivities = {
  ...
  deleteBasicTask: async (req, res) => {
    try {
      ...
      const task = await LeadTask.findByPk(id);
      if (!task)
        return res.status(200).json({
          error: "Task not found",
          statusCode: 404,
  ...
  createBasicComments: async (req, res) => {
    try {
      const { lead_id, opportunity_id, comment } = req.body;
  ...
  getAllBasicComments: async (req, res) => {
    try {
      ...
      const comments = await LeadComment.findAll({
        where: whereClause,
        include: {
          model: Users,
          as: "user",
  ...
  deleteBasicComments: async (req, res) => {
    try {
      ...
      const comment = await LeadComment.findByPk(id);
      if (!comment)
        return res.status(200).json({
