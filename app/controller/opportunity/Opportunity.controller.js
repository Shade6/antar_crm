...
exports.createDeal = async (req, res) => {
  try {
    ...
    let organization_data = {
      ...
      no_of_employees: employees?.value ?? null,
    ...
    if (req.body.organization_id) {
      ...
      organization_created_id = find_organization?.organization_id;
       ...
       if(!organization){
        return res.json({message:'please provide organization name',statusCode:400})
       }
    ...
    if (req.body.contact_id) {
      ...
      if(!first_name){
        return res.json({message:'please provide first name',statusCode:400})
       }
  ...
  //==========================================================================
    ...
    const complete_details = {
      ...
      opportunity_owner_id: owner,
      status: status?.value ?? "New",
      opportunity_name: req.body?.opportunity_name ?? "",
      opportunity_value: req.body?.opportunity_value ?? null ,
      recurring:recurring.value ?? recurring ?? null,
      created_by: req.user, // Assuming req.user.id contains the ID of the user creating the deal
