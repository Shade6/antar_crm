...
exports.find_all_role_permissions=async(req,res)=>{
  try {
    ...
    const permission = await ModulePermission.findAll({where:{role_id:role},include:[
        { model: Role, as: "role" },
        { model: Module, as: "module" },
    ]})

    res.json({message:'permission fetched',statusCode:200,data:permission??[]})
...
exports.permission_giver=async(req,res)=>{
  try {
   ...
   const find_module_perm = await ModulePermission.findOne({where:{module_permission_id:permission_id}})
   if(!find_module_perm){
    return res.json({message:'permission not found sorry!',statusCode:400})
   }
     ...
     if(sav){
      return res.json({message:'access updated',statusCode:200,data:sav})
     }else{
