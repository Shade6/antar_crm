...
exports.create_module_permission = async (req, res) => {
  try {
    ...
    const find_module_perm_name_exist = await ModulePermission.findOne({
      where: { module_perm_name: module_perm_name },
    });
    if (find_module_perm_name_exist) {
      return res.json({
        message: "module permission name is already exist !",
        statusCode: 400,
      });
    }
    ...
    return res.json({message:'module permission created successfully',statusCode:200,data:create})
  } catch (error) {
