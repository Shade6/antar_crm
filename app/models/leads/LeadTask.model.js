...
module.exports = (sequelize, Sequelize) => {
  ...
  let before = {}
  LeadTask.beforeBulkUpdate(async (options) => {
    const rowsToDelete = await LeadTask.findAll({ where: options.where });
    before = rowsToDelete[0].dataValues
  });
  LeadTask.afterBulkUpdate(async (options) => {
    const rowsToDelete = await LeadTask.findAll({ where: options.where });
    Activity.logs_entry(
      {
        before: before,
  ...
  LeadTask.beforeBulkDestroy(async (options) => {
    const rowsToDelete = await LeadTask.findAll({ where: options.where });
    Activity.logs_entry(
      rowsToDelete[0].dataValues,
      options?.context,
      "delete",
      "lead_task",
      sequelize
