...
import { update_product, create_images,get_single_product } from "@/api/userApi";
...
const handle_save = async () => {
  ...
  const res = await update_product(product_details.value);
  if (res.statusCode == 200) {
...
const fetch = async () => {
  ...
  if (res.statusCode == 200) {
    ...
    uploadedImages.value = res.data.product_image
    product_details.value.product_description=res.data?.description
  } else {
    toast.success(res.message, {
      position: "top-right",
      duration: 3000,
