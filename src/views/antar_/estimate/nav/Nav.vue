...
const route_list = ref([
  {
    label: "estimate",
    icon: "",
    route: "/antar_/estimate",
  },
]);

const isCreateRoute = () => {
  return router.currentRoute.value.path === "/antar_/estimate/create";
};

const isEditRoute = () => {
  return router.currentRoute.value.path.includes("/antar_/estimate/edit");
};

const isSingleViewRoute = () => {
  return router.currentRoute.value.path.includes("/antar_/estimate/");
};

if (isCreateRoute()) {
  route_list.value.push({
    label: "New Estimate",
    route: "/antar_/estimate/create",
  });
} else if (isEditRoute()) {
  const orgId = router.currentRoute.value.path.split("/").pop();
  route_list.value.push({
    label: `Estimate Edit ${orgId}`,
    route: router.currentRoute.value.path,
  });
} else if (isSingleViewRoute()) {
  const orgId = router.currentRoute.value.path.split("/").pop();
  route_list.value.push({
    label: `Estimate ${orgId}`,
    route: router.currentRoute.value.path,
  });
}

const handle_create = () => {
    router.push("/antar_/estimate/create");
};
