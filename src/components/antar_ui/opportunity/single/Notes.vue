...
const opportunity_route_id = route.params.id;
...
const fetch = async () => {
  const res = await get_all_basic_notes({lead_id:null,opportunity_id:opportunity_route_id});
...
const save_ = async () => {
  const data = {
    opportunity_id: opportunity_route_id,
